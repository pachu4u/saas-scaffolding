    ...
    <div className="space-y-8 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Which Riogentix feature-permission slugs each plan tier unlocks. These slugs are
          enforced by the Riogentix API on every request via the{' '}
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <ul className="mt-3 space-y-2 text-sm text-zinc-600">
          <li>
            ...
            <code className="rounded bg-zinc-100 px-1 py-0.5 text-xs">usage_locked</code> in
            every JWT via OIDC protocol mappers.
          </li>
          <li>
          ...
          <li>
            ...
            <code className="rounded bg-zinc-100 px-1 py-0.5 text-xs">
              X-Saas-Feature-Required
            </code>{' '}
            when the tenant&apos;s plan lacks the slug.
          </li>
          <li>
            <strong>UsageLockMiddleware</strong> separately blocks all write operations (POST /
            PUT / PATCH / DELETE) when{' '}
            <code className="rounded bg-zinc-100 px-1 py-0.5 text-xs">usage_locked=true</code>.
          </li>
          <li>
            Plan changes sync via <code className="rounded bg-zinc-100 px-1 py-0.5 text-xs">plan-changed</code> worker →{' '}
            <code className="rounded bg-zinc-100 px-1 py-0.5 text-xs">
