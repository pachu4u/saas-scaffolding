...
export function OnboardingWizard() {
  ...
  // Populated once step 1 actually provisions the tenant — every later step
  // targets this tenant explicitly via the x-tenant-slug header, since the
  // platform admin driving this wizard isn't browsing on that tenant's own
  // subdomain (where middleware would otherwise infer it from the host).
  ...
  function saveWorkspace() {
    ...
    startTransition(async () => {
      try {
        // This actually provisions the tenant — it is not a rename of an
        // existing one. Restricted to platform admins server-side.
        const res = await fetch('/api/tenants', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: workspaceName, slug: workspaceSlug, plan, timezone }),
        });
        const json = (await res.json()) as { id?: string; slug?: string; error?: string };
        if (!res.ok || !json.id || !json.slug) {
          setErrorMsg(json.error ?? 'Failed to create tenant');
          return;
        }
        setTenantId(json.id);
      ...
      >
        ...
        <div className="border-b px-8 py-6" style={{ borderColor: 'var(--border-light)' }}>
          ...
          <h1 className="mt-3 text-xl font-extrabold" style={{ color: 'var(--text-primary)' }}>
            {step === 'done' ? '🎉 Tenant provisioned!' : "Let's set up a new tenant."}
          </h1>
          <p className="mt-0.5 text-sm" style={{ color: 'var(--text-muted)' }}>
            {step === 'workspace' && 'First, tell us a bit about the new tenant.'}
            {step === 'invite' && 'Invite the tenant’s initial team members.'}
            {step === 'branding' && "Customize the tenant's appearance."}
            {step === 'billing' && 'Choose the plan for this tenant.'}
            {step === 'done' && 'The tenant is ready to use.'}
          </p>
        </div>
            ...
            <>
              ...
              >
                ...
                <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                  Invitees will receive an email to join the tenant. They will need to create or
          ...
          {step === 'billing' && (
            <div className="space-y-3">
              ...
              <p className="text-center text-xs" style={{ color: 'var(--text-muted)' }}>
                The plan can be changed anytime from the tenant's Billing settings.
              </p>
            </div>
          )}
            ...
            <div className="space-y-4 text-center">
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                  {workspaceName || 'The tenant'} is ready!
                </p>
